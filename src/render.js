export const CORE_DONATION = Object.freeze({
  lightningAddress: 'donate@zapm.uk',
  label: 'Support LocalRoots Water Watch',
  note: 'LocalRoots Water Watch is shared freely. If this starter helps your community, you can send sats back to the original project.'
});
...
export function renderDashboard(config, metrics = []) {
  ...
  const donationAddress = CORE_DONATION.lightningAddress;
...
<main><div class="notice">${esc(config.disclaimer || '')}</div><section class="grid">${metricCards}</section>${cameraCards ? `<h2>กล้องพื้นที่</h2><section class="grid">${cameraCards}</section>` : ''}<section class="sources"><h2>แหล่งข้อมูลที่ควรตรวจสอบร่วมกัน</h2><ul>${links}</ul></section><section class="donation" aria-labelledby="support-localroots"><h2 id="support-localroots">${esc(CORE_DONATION.label)}</h2><p>${esc(CORE_DONATION.note)}</p><div class="donation-address">⚡ ${esc(donationAddress)}</div><div class="donation-actions"><a href="lightning:${esc(donationAddress)}">Open Lightning</a><button type="button" data-copy-donation="${esc(donationAddress)}">Copy address</button></div></section></main>
...
<script>document.querySelector('[data-copy-donation]')?.addEventListener('click',async function(){const value=this.getAttribute('data-copy-donation');try{await navigator.clipboard.writeText(value);this.textContent='Copied';setTimeout(()=>this.textContent='Copy address',1600)}catch{this.textContent=value}});</script>
</body></html>`;
}
